---
title: "Changelog"
source: "https://geopandas.org/en/latest/docs/changelog.html"
...
- Expired deprecations; option `use_pygeos` which had no functionality,
  `seed` keyword in sample\_points (replaced by `rng`) (#3613)
...
- `read_postgis` will query the spatial\_ref\_sys table to determine the CRS authority
  instead of its current behaviour of assuming EPSG. In the event the spiatal\_ref\_sys
...
- Fix `read_parquet` and `read_feather` for [CVE-2023-47248](https://www.cve.org/CVERecord?id=CVE-2023-47248%3E) (#3070).
...
- Updated the distributed natural earth datasets (*naturalearth\_lowres* and
  *naturalearth\_cities*) to version 5.1 (#2555).
...
- Fix bug in `GeoDataFrame.explode()` when ‘level\_1’ is one of the column names
  (#1445).
...
- Fixed the shape of Antarctica in the included demo dataset ‘naturalearth\_lowres’
...
[Show Source](../_sources/docs/changelog.md.txt)

---

Original source: https://geopandas.org/en/latest/docs/changelog.html
