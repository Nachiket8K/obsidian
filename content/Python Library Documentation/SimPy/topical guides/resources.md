...
source: "https://simpy.readthedocs.io/en/latest/topical_guides/resources.html"
...
Shared resources are another way to model [Process Interaction](process_interaction.html). They form
a congestion point where processes queue up in order to use them.

SimPy defines three categories of resources:

- [Resources](#res-type-resource) – Resources that can be used by a limited
  number of processes at a time (e.g., a gas station with a limited number of
  fuel pumps).
- [Containers](#res-type-container) – Resources that model the production and
  consumption of a homogeneous, undifferentiated bulk. It may either be
  continuous (like water) or discrete (like apples).
- [Stores](#res-type-store) – Resources that allow the production and
...
[interrupted](process_interaction.html#interrupting-another-process) by another process. After
...
2. It may stop waiting for the request. In this case, it has to call the
   ...
   managers* (see the [Python docs](https://docs.python.org/3/reference/compound_stmts.html#with) for
   details).
...
1. [`Resource`](../api_reference/simpy.resources.html#simpy.resources.resource.Resource "simpy.resources.resource.Resource")
2. [`PriorityResource`](../api_reference/simpy.resources.html#simpy.resources.resource.PriorityResource "simpy.resources.resource.PriorityResource"), where queueing processes are sorted by priority
3. [`PreemptiveResource`](../api_reference/simpy.resources.html#simpy.resources.resource.PreemptiveResource "simpy.resources.resource.PreemptiveResource"), where processes additionally may preempt other
   processes with a lower priority

### Resource

The `Resource` is conceptually a *semaphore*. Its only parameter – apart from
the obligatory reference to an [`Environment`](../api_reference/simpy.core.html#simpy.core.Environment "simpy.core.Environment") – is its
...
Beside [`Store`](../api_reference/simpy.resources.html#simpy.resources.store.Store "simpy.resources.store.Store"), there is a [`FilterStore`](../api_reference/simpy.resources.html#simpy.resources.store.FilterStore "simpy.resources.store.FilterStore") that lets you use
a custom function to filter the objects you get out of the store and
[`PriorityStore`](../api_reference/simpy.resources.html#simpy.resources.store.PriorityStore "simpy.resources.store.PriorityStore") where items come out of the store in priority
order.
...
With a [`PriorityStore`](../api_reference/simpy.resources.html#simpy.resources.store.PriorityStore "simpy.resources.store.PriorityStore"), we can model items of differing
...
Original source: https://simpy.readthedocs.io/en/latest/topical_guides/resources.html
